...
# AES inverse S-box (对应 sbox.rs INV_SBOX)
INV_SBOX = [
...
def simulate_lg_deconfuse(data: bytes, seed: int, depth: int = 7) -> bytes:
    """模拟 LG v2.2 deconfuse（逆 S-box + 逆置换，与 wreath.rs 语义一致）"""
    n = len(data)
    layers = min(max(depth, 1), 7)
    chunk = bytearray(data)

    for li in range(layers - 1, -1, -1):  # 逆序
        s = layer_seed(seed, li)
        rng = xorshift64(s)
...
def simulate_lg_deconfuse(data: bytes, seed: int, depth: int = 7) -> bytes:
    """模拟 LG v2.2 deconfuse（逆S-box + 逆置换, 与 wreath.rs 语义一致）"""
    n = len(data)
    layers = min(max(depth, 1), 7)
    chunk = bytearray(data)
    ...
    for li in range(layers - 1, -1, -1):
        ...
        # 逆操作: INV_SBOX → XOR mask
        tmp = bytearray(n)
        for i in range(n):
            val = INV_SBOX[chunk[i]] ^ (mask[i] ^ 0x5A) & 0xFF
            tmp[inv_perm[i]] = val
        for i in range(n):
            chunk[i] = tmp[i] ^ mask[inv_perm[i]]
...
def simulate_lg_deconfuse(data: bytes, seed: int, depth: int = 7) -> bytes:
    ...
    for li in range(layers - 1, -1, -1):
        ...
        # Fisher-Yates 排列 (与 confuse 相同的排列)
        perm = list(range(n))
        for i in range(n - 1, 0, -1):
            rng = xorshift64(rng)
            j = rng % (i + 1)
        ...
        # XOR mask (与 confuse 相同的 mask)
        mask = bytearray(n)
        for i in range(n):
            rng = xorshift64(rng)
            mask[i] = rng & 0xFF

        # 逆 S-box + unmask + unpermute
        tmp = bytearray(n)
        for i in range(n):
            val = INV_SBOX[chunk[i]] ^ (mask[i] ^ 0x5A) & 0xFF
            tmp[inv_perm[i]] = val
...
def verify_deconfuse_roundtrip(plain: bytes, seed: int, depth: int) -> bool:
    """验证 confuse + deconfuse roundtrip"""
