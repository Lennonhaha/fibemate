...
软件层 SM2、ML-KEM 已完成掩码随机化、常量时间运算、TVLA 侧信道防护（N=10,000，36/36 全绿；ML-KEM-768 另完成 ADLA 双统计量测试，未检出输入值依赖时序泄露，PASS），但 FPGA 硬件 NTT、椭圆运算模块未同步实现同等防护逻辑，硬件层存在独立侧信道攻击面，软硬件安全防护体系不一致。

> 当前硬件仅完成逻辑时序验证（WNS=+9.755ns）。**预硅功耗侧信道评估能力已于 2026-09-07 具备**：预硅功耗 TVLA 流水线（B1/B2/B3 阶段）开发完成并推送，NTT 核双模式 TVLA PASS、ADLA 双统计量 PASS，配套 CI 门禁与方法论文档；这标志着「仿真/预硅层面的功耗侧信道评估能力」已落地。但**芯片级物理侧信道实测（ChipWhisperer 电流钳/EM 探针）仍待 Q4 2026 补齐**，属于「测量」缺口，而非「评估能力」缺口。

### 3.3 无第三方权威安全审计

所有生产组件仅完成自研自测、TVLA 测试、KAT 合规、形式化协议验证，未通过外部第三方密码安全审计、渗透测试、代码审计，无法规避单人开发的思维盲区与实现漏洞。这是项目评分 7.8→8.5 的最大杠杆项。

### 3.4 模糊测试 (Fuzzing) 缺口（P1）

`SECURITY.md` 明确承认 "C native addon has not undergone fuzzing"。实际 C 原生插件位于 `packages/pqc-kem/native/`（binding.gyp + fips202.c / cbd.c / indcpa.c / kem.c），承载 ML-KEM-768 的 C 端口，目前**未经任何模糊测试**，畸形/边界输入下的内存安全（越界读写、整数溢出、堆残留）未被覆盖。

> 检测方式对照（见 §10.2）：KAT 仅验证合法输入；fuzz 才覆盖恶意输入。C 原生插件缺少 fuzz 覆盖，是静默高危漏洞的潜在来源（类比历史 ML-KEM nonce 截断、decapsulate 参数顺序错误——均靠交叉验证而非 fuzz 发现）。

**计划（待启动）**：
- 引入 `libFuzzer` / `AFL++` 针对 `native/` 的边界与异常输入构造 harness（encaps / decaps / keygen 入口）
- 接入 `OSS-Fuzz` 持续模糊测试工作流（CI 门禁，long-running 覆盖）
- 与现有 liboqs C 交叉验证互补：cross-validation 验证正确性，fuzz 验证鲁棒性

状态：⚠️ **未启动** — 当前为官方承认的待办项，尚无专门工作流或已完成报告。
...
| **预硅功耗 TVLA 流水线** | ✅ 2026-09-07 完成（B1/B2/B3，NTT 核双模式 TVLA + ADLA 双统计量 PASS） | 评估能力已具备 |
| **芯片级物理 TVLA（ChipWhisperer）** | 仍无法实测功耗/EM 侧信道 | Q4 2026 计划 |
...
| ~~预硅功耗 TVLA 流水线~~ | ~~—~~ | ✅ 2026-09-07 完成（B1/B2/B3，双模式 TVLA + ADLA PASS，CI 门禁 + 方法论文档） |
| 物理 TVLA (ChipWhisperer) | Q4 2026 | 硬件侧信道实测（P0） |
| C 原生插件模糊测试 (libFuzzer/AFL++/OSS-Fuzz) | 待启动 | P1 缺口（SECURITY.md 已承认） |
