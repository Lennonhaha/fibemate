...
| Algorithm Agility | 50 | 61 | -11 |
| Key Lifecycle | 0 | 82 | **-82** |
| Protocol Coupling | 45 | 73 | -28 |
| Organizational Readiness | 35 | 70 | -35 |
| **综合** | **41** | **77.30** | **-36.30** |

> 外部人低估 36.30 分（47%）。偏差并非随机——集中在信息不可见的维度。
...
### 2. Key Lifecycle（-82）：最严重偏差——内部证据对外部人完全不可见
...
### 3. Organizational Readiness（-35）：见树不见林
...
### 4. Algorithm Agility（-11）：v4 回填后外部人转为低估

v4 回填 AlgorithmResolver + registry 包后，内部基线从 40 升至 61，外部人 50 从"高估"转为"低估"。但核心风险（表面抽象≠深层替换）依然成立。

| 子项 | 外部人判断 | 内部基线 | 分析 |
|------|-----------|---------|------|
| 接口抽象层 | "有"（选了最高选项）→ 50 | 61（AlgorithmResolver 运行时解析 + registry 包，35/35 PASS） | 外部人看不到 resolver 包；内部已补齐运行时参数解析 |
| 替换成本 | "10-100 行"→ 乐观 | gm.js 核心路径仍有 3 处硬编码（SM2 keygen/sign/encrypt） | resolver 覆盖参数解析，但 SM2 签名/加密仍硬编码 |
...
| 偏差类型 | 维度 | 偏差值 | 可纠正性 |
|---------|------|--------|---------|
| **信息不可见**（深层文件） | Key Lifecycle | -82 | ✅ 在 Step 3 展示证据文件路径后可纠正到 ~65 |
| **信息不可见**（目录广度） | Org Readiness | -35 | ✅ 展示 docs/ 文档清单后可纠正到 ~55 |
| **信息不可见**（自动化输出） | Crypto Inventory | -20 | ✅ 展示扫描器 JSON 后可纠正到 ~90 |
| **信息不可见**（resolver/回退策略） | Protocol Coupling | -28 | ✅ 展示 AlgorithmResolver 后可纠正 |
| **v4 后外部低估**（resolver 已补齐） | Algorithm Agility | -11 | ⚠️ gm.js 3 处硬编码仍待 M1/H2 解耦 |

---

## 核心结论

1. **CARS 最有价值的输出不是分数，是偏差方向。** 36.30 分的综合偏差中，绝大部分来自信息不对称（内部文件对外部人不可见）。

2. **Key Lifecycle 是关键盲区**（-82）。TLA+ 形式化验证是 FIBEMATE 最强但又最不可见的资产——`.tla` 文件不会出现在 `ls scripts/` 或 README 的 feature list 里。
...
> **一句话**：FIBEMATE 的 CARS 自评分数为 77.30（而非外部人看到的 41），但自评工具仍需展示证据文件路径——因为外部人的 41 分几乎全部来自信息不对称，而非判断差异。
