...
function generateKeypairWithSeed(seed) {
    // Domain-separated seed derivation
    // d = sha3_256("FIBEMATE-SEEDED-KEYGEN-V1" || seed || "d")
    // z = sha3_256("FIBEMATE-SEEDED-KEYGEN-V1" || seed || "z")
    const domain = new Uint8Array(new TextEncoder().encode("FIBEMATE-SEEDED-KEYGEN-V1"));
    const d = sha3_256(new Uint8Array([...domain, ...seed, 0x00]));
    const z = sha3_256(new Uint8Array([...domain, ...seed, 0x01]));
    const keyseed = sha3_512(d);
    const rho = keyseed.slice(0, 32);
    const sigma = keyseed.slice(32, 64);
    ...
    const As = matVecMul(A, s, KYBER_K);
    const tVal = vecAdd(As, e, KYBER_K);

    const pk = new Uint8Array(KYBER_PUBLICKEYBYTES);
    let off = 0;
    for (let i = 0; i < KYBER_K; i++) {
        pk.set(byteEncode(tVal[i], 12), off);
        off += 384;
    }
    pk.set(rho, off);
...
function encapsulateWithSeed(publicKey, seed) {
    const domain = new Uint8Array(new TextEncoder().encode("FIBEMATE-SEEDED-ENCAP-V1"));
    const m = sha3_256(new Uint8Array([...domain, ...seed]));
    ...
    for (let i = 0; i < KYBER_K; i++) {
        r[i] = cbd2(shake256(new Uint8Array([...m, i]), 128));
        encE1[i] = cbd2(shake256(new Uint8Array([...m, i + KYBER_K]), 128));
    }
    const encE2 = cbd2(shake256(new Uint8Array([...m, 2 * KYBER_K]), 128));

    const AT_r = matVecMul(AT, r, KYBER_K);
    const u = vecAdd(AT_r, encE1, KYBER_K);
    ...
    const v = new Int16Array(256);
    for (let i = 0; i < 256; i++) {
        v[i] = modAdd(modAdd(tTr[i], encE2[i]), mPoly[i]);
    }
