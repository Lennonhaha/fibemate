...
// TV0: string 'abc' (not hex → no auto-detect)
...
console.log('TV0 ("abc"):', SM3Hash.digestHex('abc'), tv0_ok ? 'PASS' : 'FAIL');
console.log('TV1 (64 bytes):', SM3Hash.digestHex(tv1_bytes), tv1_ok ? 'PASS' : 'FAIL');
// NOTE: JS digestHex auto-detects hex strings — Uint8Array required for binary vectors

// KAT cross-validation
const katPath = fs.existsSync('../../packages/sm2-ref/test/kat/sm3-KAT.json')
  ? '../../packages/sm2-ref/test/kat/sm3-KAT.json'
  : '../sm2-ref/test/kat/sm3-KAT.json';
const kat = JSON.parse(fs.readFileSync(katPath, 'utf8'));
console.log('Loaded ' + kat.length + ' KAT vectors');
...
console.log('  KAT (' + kat.length + ' vectors): ' + pass + '/' + kat.length + ' PASS');
