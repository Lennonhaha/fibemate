...
FIBEMATE 是全球首个**从浏览器到 FPGA** 全栈贯通的后量子密码学教育验证平台——NIST PQC 三算法全实现 + 国密 SM2/3/4 + 20 个交互式可视化分析工具 + 100+ 份时间戳存证。
...
| **可视化断层** | PQC 安全评估依赖命令行工具 | 20 个交互式仪表盘，浏览器即用 |
...
### 20 个交互式可视化（含 ML-KEM 密钥封装流程动画、ML-DSA 签名验证流程动画、SLH-DSA 签名流程动画、Double Ratchet 混合棘轮动画、TLS 1.3 混合握手序列图）
...
| 📈 趋势 | 评分时间轴 | CARS 63.50→75.20 / IBM 39→63 |
...
| 框架 | 综合分 | 各维度 |
|:---|:---|:---|
| **CARS** | **75.20** | 加密资产盘点 90 · 算法敏捷性 61 · 密钥生命周期 82 · 协议耦合 73 · 组织准备度 78 |
...
| 版本 | 时间 | 重点 |
|:---|:---|:---|
| v3.3 | 2026-08-31 | 开源首发 · 20 可视页 · CARS 75.20 · IBM 63.70 · PQC 部署验证器 (Model 2) |
