...
const E2EEDisplay = (() => {
  // ── 加密状态枚举 ──
  const STATUS = {
    NONE:           { level: 0, icon: '🔓', label: '未加密',    color: 'var(--danger)',  desc: 'Messages are not encrypted' },
    HANDSHAKE:      { level: 1, icon: '⏳', label: '握手',  color: '#FFA502',       desc: 'Establishing secure session...' },
    ENCRYPTED:      { level: 2, icon: '🔒', label: '国密',       color: 'var(--accent)',  desc: '国密加密 (SM2 + SM4-αGCM)' },
    POST_QUANTUM:   { level: 3, icon: '🛡️', label: 'PQ-国密',   color: '#6C5CE7',       desc: '后量子国密 (ML-KEM-768 + SM2)' },
    QUANTUM_ENHANCED: { level: 4, icon: '⚛️', label: 'Q-国密',  color: '#00CEC9',       desc: '量子增强国密 (QKD + ML-KEM-768 + SM2)' }
  };
  ...
  async function updateStatus(peerId) {
    ...
    if (Quantum && Quantum.isEnabled?.()) {
      ...
      qStatus.desc = '量子增强国密 (QKD + ML-KEM-768 + SM2)';
    ...
    // 标准加密
    const securityStatus = await Crypto.getSecurityStatus?.(peerId);
    _setStatus(bar, icon, text, detail, STATUS.ENCRYPTED, `${securityStatus?.curve || 'SM2'} · ${(securityStatus?.messagesSent || 0) + (securityStatus?.messagesReceived || 0)} msgs`);
  }
    ...
    panel.innerHTML = `
      <div class="e2ee-detail-header">
        ...
        <div>
          ...
          <div class="e2ee-detail-subtitle">${e2eeStatus.desc.replace('End-to-end encrypted', '国密加密').replace('X3DH + Double Ratchet', 'SM2 + SM4-αGCM').replace('Post-quantum E2EE', '后量子国密').replace('ML-KEM-768 + X3DH', 'ML-KEM-768 + SM2').replace('Quantum-enhanced E2EE', '量子增强国密')}</div>
        </div>
      </div>

      <div class="e2ee-info-row">
        <span class="e2ee-info-label">Protocol</span>
        <span class="e2ee-info-value secure">${protocolInfo.protocol || 'SM4-αGCM'}</span>
      </div>
      <div class="e2ee-info-row">
        <span class="e2ee-info-label">Key Exchange</span>
        <span class="e2ee-info-value secure">SM2 (国密椭圆曲线)</span>
      </div>
      <div class="e2ee-info-row">
        <span class="e2ee-info-label">Symmetric Cipher</span>
        <span class="e2ee-info-value">SM4-CBC + SM3-HMAC (α-GCM)</span>
      </div>
      <div class="e2ee-info-row">
        <span class="e2ee-info-label">Curve</span>
        <span class="e2ee-info-value">${protocolInfo.curve || 'SM2'}</span>
      </div>
      <div class="e2ee-info-row">
      ...
      <div class="e2ee-safety-number">
        <div class="e2ee-safety-number-label">Safety Number (SM3)</div>
      ...
      <div class="e2ee-protocol-layers">
        <div class="e2ee-protocol-layer">
          ...
          <span class="e2ee-protocol-layer-name">SM4-αGCM (认证加密)</span>
          <span class="e2ee-protocol-layer-status active">ACTIVE</span>
        </div>
        <div class="e2ee-protocol-layer">
          <span class="e2ee-protocol-layer-icon">🔑</span>
          <span class="e2ee-protocol-layer-name">SM2 密钥协商</span>
          <span class="e2ee-protocol-layer-status active">ACTIVE</span>
        </div>
        <div class="e2ee-protocol-layer">
          <span class="e2ee-protocol-layer-icon">📊</span>
          <span class="e2ee-protocol-layer-name">SM3 哈希/完整性</span>
          <span class="e2ee-protocol-layer-status active">ACTIVE</span>
        </div>
        <div class="e2ee-protocol-layer">
          <span class="e2ee-protocol-layer-icon">🛡️</span>
          <span class="e2ee-protocol-layer-name">ML-KEM-768 (后量子)</span>
          <span class="e2ee-protocol-layer-status ${PQ?.isAvailable?.() ? 'active' : 'inactive'}">${PQ?.isAvailable?.() ? 'ACTIVE' : 'INACTIVE'}</span>
        </div>
        <div class="e2ee-protocol-layer">
          <span class="e2ee-protocol-layer-icon">⚛️</span>
          <span class="e2ee-protocol-layer-name">量子增强</span>
