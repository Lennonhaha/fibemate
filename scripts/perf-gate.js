...
#!/usr/bin/env node
/**
 * FIBEMATE 性能门禁 �?最小补�? * 用法: node scripts/perf-gate.js [--baseline baseline.json]
 * 退出码: 0 = PASS, 1 = FAIL
 */
...
// 性能基线（单�? ms/op�?const DEFAULT_BASELINE = {
...
// 退化阈�?const REGRESSION_THRESHOLD = 1.20; // 20% 退化即报警
...
function checkRegression(result, baseline) {
  ...
  if (maxRatio >= HARD_FAIL_THRESHOLD) {
    return { status: 'FAIL', reason: `${maxRatio.toFixed(2)}x退�?阈�?{HARD_FAIL_THRESHOLD}x)`, p95Ratio, meanRatio };
  }
  if (maxRatio >= REGRESSION_THRESHOLD) {
    return { status: 'WARN', reason: `${maxRatio.toFixed(2)}x退�?阈�?{REGRESSION_THRESHOLD}x)`, p95Ratio, meanRatio };
  }
...
async function main() {
  ...
  // 输出报告
  console.log('\n┌─────────────────────┬──────────┬──────────┬──────────┬──────────�?);
  console.log('�?操作                �?均�?ms) �?p95(ms)  �?基线     �?状�?    �?);
  console.log('├─────────────────────┼──────────┼──────────┼──────────┼──────────�?);
  ...
  for (const r of results) {
    ...
    if (r.reason && r.status !== 'PASS') {
      console.log(`�?${''.padEnd(19)} �?退�? ${r.reason.padEnd(32)} │`);
    }
  }
  ...
  // 汇�?  console.log('\n───────────────────────────────────────────────────────────────');
  console.log(`  结果: ${results.length} 项测�? ${failCount} 失败, ${warnCount} 警告`);
