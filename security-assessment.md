...
> **⚠️ 关于 §3 攻击数据的说明**：§3 中引用的「5,913 冲突」「1,000/1,000 roundtrip」等数据来源于 Python 模拟层（`simulate_lg_confuse` = XOR + S-box + Fisher-Yates）对 LG 攻击框架的验证，**未经真实 WASM 验证**。这些数据用于证明攻击框架的逻辑可行性，不代表真实 LG WASM 实现的攻击面结论。真实 WASM 的攻击面参见 §4。
