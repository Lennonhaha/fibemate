...
// FIBEMATE P-256 Double Ratchet Base Class
// Algorithm: Signal Protocol Double Ratchet (standard, self-implemented)
...
const DoubleRatchet = (() => {
   ...
   * @param {Uint8Array} rootKey - 32-byte root key from X3DH
   * @param {Uint8Array} remoteDHPublic - Bob's signed pre-key (raw P-256 public key)
   ...
   * @param {Uint8Array} rootKey - 32-byte root key from X3DH
   * @param {CryptoKeyPair} selfDH - Bob's current DH key pair (signed pre-key)
   * @returns {Object} ratchet state
   */
  ...
  /**
   * Encrypt a message.
   * @param {Object} state - ratchet state (mutated)
   * @param {string} plaintext - message to encrypt
   * @returns {{header: Object, ciphertext: number[], iv: number[]}}
   */
   ...
   * Decrypt a message.
   * @param {Object} state - ratchet state (mutated)
   * @param {Object} header - ratchet header {dh, pn, n}
   * @param {Uint8Array|number[]} ciphertext
   * @param {Uint8Array|number[]} iv
  ...
  /**
   * X3DH key agreement (full 4-DH variant).
   * Returns { rootKey, ephemeralKey, identityKey } for Alice;
   * Bob uses initAsReceiver with the derived rootKey.
   */
